...
export default class kult4ePCsheet extends ActorSheet{
    ...
    static get defaultOptions() {
      return mergeObject(super.defaultOptions, {
        classes: ["kult4e", "sheet", "actor"],
        template: "systems/kult4e/templates/actor/actor-sheet.html",
        width: 900,
        height: 900,
...
super.activateListeners(html)
  ...
  html.find('.move-roll').click(ev =>{
    const li = $(ev.currentTarget).parents(".item-name");
    const item = this.actor.getOwnedItem(li.data("itemId"));
    console.log("move Data:", item.data);
  ...
  html.find('.stability-minus').click(ev =>{
    ...
    {ChatMessage.create({ content: `${this.actor.name} traci nad sobą kontrolę i wydaje się być mniej stabilny.`, speaker: ChatMessage.getSpeaker({alias: this.name})});
  } }
  else {ui.notifications.warn("Postać jest zdruzgotana!")}
  ...
  html.find('.stability-plus').click(ev =>{
    ...
    let stability_new = stability_current - 1;
    this.actor.update({'data.stability.value': stability_new});
    {ChatMessage.create({ content: `${this.actor.name} zapanował nad sobą i wydaje się być bardziej stabilny.`, speaker: ChatMessage.getSpeaker({alias: this.name})});
  }
}
    else{ui.notifications.warn("Postać jest całkowicie stabilna")}
  });
